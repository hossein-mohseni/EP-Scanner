...
async def ping_ip(semaphore, ip, num_pings=8):
...
async def main():
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        for i, (ip, _) in enumerate(top_5):
            try:
                process = await asyncio.wait_for(loop.run_in_executor(executor, run_warp_plus, ip), timeout=10)
                if process:
                    try:
                    ...
                    if new_ip and new_ip != original_ip:
                        print(fg(15) + f"[{i + 1}] " + fg(15) + "- " + fg(56) + "IP: " + fg(163) + f"{ip}:2408" + fg(15) + " | " + fg(56) + "warp-plus Test: " + fg(70) + "PASS" + attr(0))
                    else:
                        print(fg(15) + f"[{i + 1}] " + fg(15) + "- " + fg(56) + "IP: " + fg(163) + f"{ip}:2408" + fg(15) + " | " + fg(56) + "warp-plus Test: " + fg(160) + "Failed" + attr(0))
